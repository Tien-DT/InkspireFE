    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Dưới đây là kết quả đánh giá chi tiết từ AI.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex items-center gap-4">
                ...
                <p>Có đạt yêu cầu không: {evaluationData.meetsRequirements ? <Badge className="bg-green-500">Đạt</Badge> : <Badge variant="destructive">Không đạt</Badge>}</p>
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <ul className="list-disc list-inside">
                {evaluationData.strengths.map((item, index) => <li key={index}>{item}</li>)}
              </ul>
            </div>
            <div>
              <h3 className="font-semibold">Điểm yếu</h3>
              <ul className="list-disc list-inside">
                {evaluationData.weaknesses.map((item, index) => <li key={index}>{item}</li>)}
              </ul>
            </div>
            <div>
              <h3 className="font-semibold">Đề xuất</h3>
              <ul className="list-disc list-inside">
                {evaluationData.suggestions.map((item, index) => <li key={index}>{item}</li>)}
              </ul>
            </div>
             <div>
              <h3 className="font-semibold">Các phần còn thiếu</h3>
              <ul className="list-disc list-inside">
                {evaluationData.missingElements.map((item, index) => <li key={index}>{item}</li>)}
              </ul>
            </div>
          </div>
        </div>

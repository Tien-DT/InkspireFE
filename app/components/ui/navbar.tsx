...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        'flex flex-1 flex-row items-center justify-center space-x-2 font-medium text-zinc-600 transition duration-200 hover:text-zinc-800 lg:space-x-2 text-base font-sans',
        className
      )}
    >
      {items.map((item, idx) => (
        ...
        >
          {hovered === idx && (
            <motion.div
              layoutId='hovered'
              className='absolute inset-0 h-full w-full rounded-full bg-gray-300 dark:bg-neutral-900'
            />
          )}
...
export const MobileNavHeader = ({ children, className }: MobileNavHeaderProps) => {
  return <div className={cn('flex w-full flex-row items-center justify-between px-4 py-2', className)}>{children}</div>
}
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0, y: -10 }}
          className={cn(
            'absolute inset-x-0 top-full z-40 flex w-full flex-col items-start justify-start gap-4 rounded-lg bg-white px-4 py-8 shadow-[0_0_24px_rgba(34,_42,_53,_0.06),_0_1px_1px_rgba(0,_0,_0,_0.05),_0_0_0_1px_rgba(34,_42,_53,_0.04),_0_0_4px_rgba(34,_42,_53,_0.08),_0_16px_68px_rgba(47,_48,_55,_0.05),_0_1px_0_rgba(255,_255,_255,_0.1)_inset] dark:bg-neutral-950',
            className
          )}
        >
          {children}
...
export const NavbarLogo = ({ href = '/', logo, label }: { href?: string; logo: string; label: string }) => {
  return (
    <a
      href={href}

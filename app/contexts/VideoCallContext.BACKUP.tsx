...
export const VideoCallProvider = ({ children }: VideoCallProviderProps) => {
  ...
  const initiateCall = useCallback(
    async (conversationId: string, receiverUserId: string, receiverName: string, callType: CallType) => {
      ...
      try {
        ...
        console.log('[WebRTC] Creating offer with explicit constraints...')
        const offer = await pc.createOffer({
          offerToReceiveAudio: true,
          offerToReceiveVideo: callType === 'video'
        })
        
        console.log('[WebRTC] Offer created, checking SDP...')
        console.log('[WebRTC] Offer has audio:', offer.sdp?.includes('m=audio'))
        console.log('[WebRTC] Offer has video:', offer.sdp?.includes('m=video'))
  ...
  const acceptCall = useCallback(async () => {
    try {
      ...
      console.log('[WebRTC] Creating answer with explicit constraints...')
      const answer = await pc.createAnswer({
        offerToReceiveAudio: true,
        offerToReceiveVideo: callState.callType === 'video'
      })
      
      console.log('[WebRTC] Answer created, checking SDP...')
      console.log('[WebRTC] Answer has audio:', answer.sdp?.includes('m=audio'))
      console.log('[WebRTC] Answer has video:', answer.sdp?.includes('m=video'))
  ...
  const handleCallAnswer = useCallback(async (answer: CallAnswer) => {
    ...
    try {
      ...
      // Manually trigger remote stream setup if ontrack didn't fire
      console.log('[WebRTC] Checking for remote stream...')
      const remoteStream = new MediaStream()
      receivers.forEach(receiver => {
        if (receiver.track) {
          console.log('[WebRTC] Adding remote track to stream manually:', receiver.track.kind)
          remoteStream.addTrack(receiver.track)
        }
      })
      
      if (remoteStream.getTracks().length > 0) {
        console.log('[WebRTC] Setting remote stream manually with', remoteStream.getTracks().length, 'tracks')
        ...
        console.log('[WebRTC] No remote tracks found yet')
        setCallState((prev) => ({
          ...prev,
          status: CallStatusEnum.Connecting
        }))
